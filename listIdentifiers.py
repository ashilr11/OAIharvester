# Ashil Ramjee
# Python OAI harvester for List Identifiers
...
if __name__ == "__main__":
    ...
    parser.add_option("-f", "--from", dest="fromDate", help="harvest identifiers from this date yyyy-mm-dd")
    parser.add_option("-u", "--until", dest="until", help="harvest identifiers until this date yyyy-mm-dd")
    ...
    print "Writing identifiers to %s from archive %s" % (outFileName, serverString)
    ...
    print "Using url:%s" % serverString + '?ListIdentifiers' + verbOpts

    data = getFile(serverString, 'ListIdentifiers' + verbOpts)
    ...
    while data:
        ...
        for (event, node) in events:
            if event == "START_ELEMENT" and node.tagName == 'header':
        ...
        if not mo:
            break
        data = getFile(serverString, "ListIdentifiers&resumptionToken=%s" % mo.group(1))

    ofile.write('\n</repository>\n'), ofile.close()

    print "\nRead %d bytes (%.2f compression)" % (nDataBytes, float(nDataBytes) / nRawBytes)

    print "Wrote out %d identifiers" % recordCount
